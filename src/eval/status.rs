...
fn next_actions(phase: ControllerClaimStatusPhase) -> Vec<String> {
    match phase {
        ...
        ControllerClaimStatusPhase::AwaitingLiveEvidence => vec![
            ...
            "For local decoders, start with plan-controller-offline-run, export and verify the prompt bundle, save response files, then run score-controller-responses for each bucket.".to_string(),
