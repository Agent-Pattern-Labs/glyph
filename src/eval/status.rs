...
fn next_actions(phase: ControllerClaimStatusPhase) -> Vec<String> {
    match phase {
        ...
        ControllerClaimStatusPhase::AwaitingLiveEvidence => vec![
            ...
            "For local decoders, start with plan-controller-offline-run, export and verify the prompt bundle, export-controller-offline-queue, verify-controller-offline-queue, save response files, run check-controller-offline-responses, then score-controller-responses for each bucket.".to_string(),
